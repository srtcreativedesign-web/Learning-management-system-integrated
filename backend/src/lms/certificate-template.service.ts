...
export class CertificateTemplateService {
  ...
  /**
   * Menampilkan semua sertifikat kelulusan yang telah diterbitkan
   * bersumber dari pengerjaan kuis yang lulus (EmployeeQuizAttempt dengan is_passed: true).
   */
  async getIssuedCertificates() {
    const passedAttempts = await (this.prisma as any).employeeQuizAttempt.findMany({
    ...
    return passedAttempts.map((attempt: any) => {
      const courseTitle =
  ...
  /**
   * Verifikasi keaslian sertifikat berdasarkan ID Attempt atau Nomor Sertifikat
   */
  async verifyCertificate(identifier: string) {
    ...
    if (!found) {
      throw new NotFoundException('Sertifikat tidak ditemukan atau belum terverifikasi');
    }
